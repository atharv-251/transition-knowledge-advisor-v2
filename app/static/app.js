...
function render() {
  ...
  byId("schedule-body").innerHTML = schedule.html; byId("schedule-records").textContent = `${transition.schedule.length} sessions`; pagination("schedule", transition.schedule.length);
...
document.addEventListener("click", async (event) => { const target = event.target; if (target.dataset.view) { setView(target.dataset.view); } if (target.dataset.pageView) { pageState[target.dataset.pageView] = Number(target.dataset.pageNumber); render(); } if (target.dataset.workspaceView) { if (target.dataset.workspaceView === "select") { try { await renderTransitionList(); } catch (error) { notify(error.message); } } setWorkspaceView(target.dataset.workspaceView); } if (target.dataset.transitionId) { try { await loadTransition(target.dataset.transitionId); setWorkspaceView("current"); } catch (error) { notify(error.message); } } });
init();